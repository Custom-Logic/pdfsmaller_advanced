...
import { AuthManager } from './modules/auth-manager.js'; // Uncomment if this module exists
...
export class MainIntegration {
  ...
  setupSideNavListener() {
    ...
    if (sideNavToggle && sideNav && navOverlay) {
      sideNavToggle.addEventListener('click', () => {
        sideNav.classList.toggle('active');
        sideNavToggle.classList.toggle('active');
        const isExpanded = sideNav.classList.contains('active');
        sideNavToggle.setAttribute('aria-expanded', isExpanded);
        sideNav.setAttribute('aria-hidden', !isExpanded);
      });
  ...
  setupAuthModal() {
    ...
    const closeButtons = authModal?.querySelectorAll('[data-action="close-auth-modal"]');
    const authPanel = document.getElementById('modalAuthPanel');
    
    if (authModal && modalOverlay && closeButtons && authPanel) {
      ...
      // Listen for auth events from the auth panel
      authPanel.addEventListener('auth:login', (event) => {
        this.handleLogin(event.detail);
        closeModal();
      });
        
      authPanel.addEventListener('auth:register', (event) => {
        this.handleRegister(event.detail);
        closeModal();
      });
        
      authPanel.addEventListener('auth:logout', () => {
        this.handleLogout();
        closeModal();
      });
  ...
  handleAuthAction(action) {
    ...
    if (!authModal || !authPanel) return;
    
    switch (action) {
      case 'show-login':
        authPanel.setMode('login');
        this.showAuthModal();
        break;
      case 'show-register':
        authPanel.setMode('register');
        this.showAuthModal();
        break;
      case 'show-profile':
        // Handle profile navigation
        if (window.mainIntegration) {
          window.mainIntegration.switchTab('profile');
        }
        break;
      case 'logout':
        this.handleLogout();
  ...
  handleAuthStateChange(authDetail) {
    ...
    if (guestSection && userSection) {
      if (this.isAuthenticated) {
        ...
        if (userName && userPlan && this.currentUser) {
          userName.textContent = this.currentUser.name || 'User';
          userPlan.textContent = this.getPlanDisplayName(this.currentUser);
        }
      } else {
  ...
  setupEventListeners() {
    ...
    // CORRECTED: Listen for auth state changes on the document
